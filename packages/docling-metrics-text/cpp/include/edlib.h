...
typedef enum {
  ...
  EDLIB_MODE_NW,
  /**
   * Prefix method. Similar to global method, but with a small twist - gap at query end is not
   * penalized. What that means is that deleting elements from the end of second sequence is "free"!
   * For example, if we had "AACT" and "AACTGGC", edit distance would be 0, because removing "GGC"
   * from the end of second sequence is "free" and does not count into total edit distance. This
   * method is appropriate when you want to find out how well first sequence fits at the beginning
   * of second sequence.
   */
  EDLIB_MODE_SHW,
  /**
   * Infix method. Similar as prefix method, but with one more twist - gaps at query end and start
   * are not penalized. What that means is that deleting elements from the start and end of second
   * sequence is "free"! For example, if we had ACT and CGACTGAC, edit distance would be 0, because
   * removing CG from the start and GAC from the end of second sequence is "free" and does not count
   * into total edit distance. This method is appropriate when you want to find out how well first
   * sequence fits at any part of second sequence. For example, if your second sequence was a long
   * text and your first sequence was a sentence from that text, but slightly scrambled, you could
   * use this method to discover how scrambled it is and where it fits in that text. In
   * bioinformatics, this method is appropriate for aligning read to a sequence.
   */
  EDLIB_MODE_HW
} EdlibAlignMode;
...
typedef struct {
   ...
   * EDLIB_TASK_DISTANCE - find edit distance and end locations of optimal alignment paths in
   * target. EDLIB_TASK_LOC - find edit distance and start and end locations of optimal alignment
   * paths in target. EDLIB_TASK_PATH - find edit distance, alignment path (and start and end
   ...
   * This way you can extend edlib's definition of equality (which is that each character is equal
   * only to itself). This can be useful if you have some wildcard characters that should match
   * multiple other characters, or e.g. if you want edlib to be case insensitive. Can be set to NULL
   * if there are none.
   */
...
typedef struct {
   ...
   * If you do not free whole result object using edlibFreeAlignResult(), do not forget to use
   * free().
   */
  int *endLocations;
 ...
 * Through config parameter, this function supports different alignment methods (global, prefix,
 * infix), as well as different modes of search (tasks). It always returns edit distance and end
 * locations of optimal alignment in target. It optionally returns start locations of optimal
 ...
 * @return  Result of alignment, which can contain edit distance, start and end locations and
 * alignment path. Make sure to clean up the object using edlibFreeAlignResult() or by manually
 * freeing needed members.
 */
EDLIB_API EdlibAlignResult edlibAlign(const char *query, int queryLength, const char *target,
