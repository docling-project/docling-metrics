 ...
 * By default each character is always equal only to itself, but you can also provide additional
 * equalities.
 */
...
extern "C" EdlibAlignResult edlibAlign(const char *const queryOriginal, const int queryLength,
  ...
  string alphabet =
      transformSequences(queryOriginal, queryLength, targetOriginal, targetLength, &query, &target);
  ...
  do {
    if (config.mode == EDLIB_MODE_HW || config.mode == EDLIB_MODE_SHW) {
      myersCalcEditDistanceSemiGlobal(Peq, W, maxNumBlocks, queryLength, target, targetLength, k,
  ...
  if (result.editDistance >= 0) { // If there is solution.
    ...
    if (config.task == EDLIB_TASK_LOC || config.task == EDLIB_TASK_PATH) {
      ...
      if (config.mode == EDLIB_MODE_HW) { // If HW, I need to calculate start locations.
        ...
        Word *rPeq =
            buildPeq(static_cast<int>(alphabet.size()), rQuery, queryLength, equalityDefinition);
        for (int i = 0; i < result.numLocations; i++) {
          int endLocation = result.endLocations[i];
          if (endLocation == -1) {
            // NOTE: Sometimes one of optimal solutions is that query starts before target, like
            // this:
            //                       AAGG <- target
            //                   CCTT     <- query
 ...
 * Bit i of Peq[s * maxNumBlocks + b] is 1 if i-th symbol from block b of query equals symbol s,
 * otherwise it is 0. NOTICE: free returned array with delete[]!
 */
static inline Word *buildPeq(const int alphabetLength, const unsigned char *const query,
  ...
  // Build Peq (1 is match, 0 is mismatch). NOTE: last column is wildcard(symbol that matches
  // anything) with just 1s
 ...
 * @param [in] block
 * @param [out] dest  Array into which cell values are written. Must have size of at least
 * WORD_SIZE.
 */
  ...
  *numPositions_ = 0;
  ...
  for (int c = 0; c < targetLength; c++) { // for each column
    ...
    if ((lastBlock < maxNumBlocks - 1) &&
        (blocks[bl].score - hout <= k)              // bl is pointing to last block
      ...
      blocks[bl].score =
          blocks[bl - 1].score - hout + WORD_SIZE +
          calculateBlock(blocks[bl].P, blocks[bl].M, *Peq_c, hout, blocks[bl].P, blocks[bl].M);
    } else {
    ...
    if (lastBlock == maxNumBlocks - 1) {
      ...
      if (colScore <= k) { // Scores > k dont have correct values (so we cannot use them), but are
                           // certainly > k.
 ...
 * @param [out] targetStopPosition  If set to -1, whole calculation is performed normally, as
 * expected. If set to p, calculation is performed up to position p in target (inclusive) and column
 * p is returned as the only column in alignData.
 * @return Status.
 */
static int myersCalcEditDistanceNW(const Word *const Peq, const int W, const int maxNumBlocks,
  ...
  const int STRONG_REDUCE_NUM =
      2048; // TODO: Choose this number dinamically (based on query and target lengths?), so it does
            // not affect speed of computation
  ...
  for (int c = 0; c < targetLength; c++) { // for each column
    ...
    // NOTICE: I add W when in last block because it is actually result from W cells to the left and
    // W cells up.
    ...
    //---------- Adjust number of blocks according to Ukkonen ----------//
    //--- Adjust last block ---//
    // If block is not beneath band, calculate next block. Only next because others are certainly
    // beneath band.
    if (lastBlock + 1 < maxNumBlocks &&
        !( // score[lastBlock] >= k + WORD_SIZE ||  // NOTICE: this condition could be satisfied if
           // above block also!
      ...
      blocks[bl].P = static_cast<Word>(-1); // All 1s
      blocks[bl].M = static_cast<Word>(0);
      int newHout = calculateBlock(blocks[bl].P, blocks[bl].M, Peq_c[lastBlock], hout, blocks[bl].P,
                                   blocks[bl].M);
    ...
    // NOTE: Condition used here is more loose than the one from the article, since I simplified the
    // max() part of it. I could consider adding that max part, for optimal performance.
...
/**
 * Finds one possible alignment that gives optimal score by moving back through the dynamic
 * programming matrix, that is stored in alignData. Consumes large amount of memory: O(queryLength *
 * targetLength).
 * @param [in] queryLength  Normal length, without W.
 * @param [in] targetLength  Normal length, without W.
 * @param [in] bestScore  Best score.
 * @param [in] alignData  Data obtained during finding best score that is useful for finding
 * alignment.
...
static int obtainAlignmentTraceback(const int queryLength, const int targetLength,
  ...
  *alignmentLength = 0;
  ...
  // We set initial values of lP and lM to 0 only to avoid compiler warnings, they should not affect
  // the calculation as both lP and lM should be initialized at some moment later (but compiler can
  // not detect it since this initialization is guaranteed by "business" logic).
  Word lP = 0, lM = 0;
  if (thereIsLeftBlock) {
      ...
      if (blockPos == 0) { // If entering new (upper) block
        if (b == 0) {      // If there are no cells above (only boundary cells)
          ...
          if (c > 0 && b >= alignData->firstBlocks[c - 1] && b <= alignData->lastBlocks[c - 1]) {
            thereIsLeftBlock = true;
            lP = alignData
                     ->Ps[(c - 1) * maxNumBlocks + b]; // TODO: improve this, too many operations
            ...
            // handle this correctly then? Are l and ul score set correctly? I should check that /
            // refactor this.
          }
        }
      } else {
  ...
  *alignment =
      static_cast<unsigned char *>(realloc(*alignment, (*alignmentLength) * sizeof(unsigned char)));
 ...
 * It will split problem into smaller problems using Hirschberg's algorithm and when they are small
 * enough, it will solve them using traceback algorithm.
 * @param [in] query
...
static int obtainAlignment(const unsigned char *const query, const unsigned char *const rQuery,
  ...
  // and it could also be done for alignments - we could have one big array for alignment that would
  // be sparsely populated by each of steps in recursion, and at the end we would just consolidate
  // those results.
...
/**
 * Finds one possible alignment that gives optimal score (bestScore).
 * Uses Hirschberg's algorithm to split problem into two sub-problems, solve them and combine them
 * together.
 * @param [in] query
 * @param [in] rQuery  Reversed query.
 * @param [in] queryLength
...
static int obtainAlignmentHirschberg(const unsigned char *const query,
  ...
  int rightHalfCalcStatus = myersCalcEditDistanceNW(rPeq, W, maxNumBlocks, queryLength, rTarget,
                                                    ...
                                                    false, &alignDataRightHalf, rightHalfWidth - 1);
  ...
  // If there is padding at the beginning of scoresRight (that can happen because of reversing that
  // we do), move pointer forward to remove the padding (that is why we remember originalStart).
  ...
  int ulStatusCode = obtainAlignment(query, rQuery + lrHeight, ulHeight, target, rTarget + lrWidth,
                                     ulWidth, equalityDefinition, alphabetLength, leftScore,
                                     &ulAlignment, &ulAlignmentLength);
  unsigned char *lrAlignment = NULL;
  int lrAlignmentLength;
  int lrStatusCode = obtainAlignment(query + ulHeight, rQuery, lrHeight, target + ulWidth, rTarget,
                                     lrWidth, equalityDefinition, alphabetLength, rightScore,
...
/**
 * Takes char query and char target, recognizes alphabet and transforms them into unsigned char
 * sequences where elements in sequences are not any more letters of alphabet, but their index in
 * alphabet. Most of internal edlib functions expect such transformed sequences. This function will
 * allocate queryTransformed and targetTransformed, so make sure to free them when done. Example:
 ...
 * @return  Alphabet as a string of unique characters, where index of each character is its value in
