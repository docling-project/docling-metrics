...
import pytest
from docling_metric_teds.docling_metric_teds_cpp import TEDSManager, TEDSampleEvaluation, TEDSDatasetEvaluation
...
def test_cpp_bindings():
    ...
    # Evaluate sample
    id = "s1"
    sample_evaluation: TEDSampleEvaluation = teds_manager.evaluate_sample(id, gt_bracket, pred_bracket)

    # Assertions
    assert sample_evaluation.error_id == 0, "Expected no error for valid brackets"
    assert math.isclose(sample_evaluation.teds, 0.970588, rel_tol=1e-6), "Wrong TEDS score for valid bracket"
    ...
    broken_eval: TEDSampleEvaluation = teds_manager.evaluate_sample("s2", broken_bracket, pred_bracket)
