...
from docling_metrics_table.utils.grits import html_to_cells, cells_to_html
...
def test_cells_input():
    ...
    def html_to_cells_input(html_str: str) -> list[TableMetricCell]:
        cells = [
    ...
    for stem, test_data in all_test_data.items():
        ...
        print("\n=== Cells Input ===")
        cells_a = html_to_cells_input(test_data["gt_html"])
        cells_b = html_to_cells_input(test_data["pred_html"])
...
def test_html_to_cells():
    r"""
    It seems that the HTML we produce (TEDS, OTSL) uses tbody tags but
    the conversion of Cell to HTML does not generate tbody tags
    """
    all_test_data: dict[str, dict[str, str]] = load_test_data()
    table_metric = TableMetric(metrics=[TableMetricKind.TEDS])

    for stem, test_data in all_test_data.items():
        gt_html = test_data["gt_html"]
        pred_html = test_data["pred_html"]

        gt_cells = html_to_cells(gt_html)
        pred_cells = html_to_cells(pred_html)

        gt_html2 = cells_to_html(gt_cells)
        pred_html2 = cells_to_html(pred_cells)
        print(f"A1: {gt_html}")
        print(f"A2: {gt_html2}")
        print(f"B1: {pred_html}")
        print(f"B2: {pred_html2}")
