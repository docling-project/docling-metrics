...
def test_cells_input():
    ...
    - The conversion from HTML to cells lacks the location.
      Therefore all bboxes are grounded to a default value
    - The conversionn from cells to HTML (happens inside the TableMetric), lacks the tbody tag.
      Therefore the generated bracket has one element less and the TEDS score is a bit different.
    """
    all_test_data: dict[str, dict[str, str]] = load_test_data()
    table_metric = TableMetric(metrics=[TableMetricKind.TEDS])
    ...
    for stem, test_data in all_test_data.items():
        ...
        # Notice: We know that that the HTML generated by the Cells does NOT produce tbody tags
        expected_cells_tree_a_size = config["tree_a_size"] - 1
        expected_cells_tree_b_size = config["tree_b_size"] - 1
