...
def run_eval(predicted_name, gt_name, label):
    ...
    for i, (p, g) in enumerate(zip(predicted, gt)):
        ...
        samples.append(SmilesInputSample(id=str(i), predicted_smiles=p, gt_smiles=g, is_markush=True))
...
run_eval("test_predicted_smiles_list.json", "test_gt_smiles_list.json", "Standard Dataset")
